#!/usr/bin/env python3
"""Generate smoke v6 sigma sweep env files (18 total: 2 modes × 9 sigmas).
...
ICS level 90 is used uniformly across all sigmas.

Comparable to smoke-v5 (HW/HD): same trace counts, same SASCA params,
corrected leakage scales.

Note: TRACES_DIR is not set in env files — run_sandboxes.sh injects it
automatically as traces_smoke_v6_{mode}_sigma{X} in the remote base dir.
"""
import os
...
TEMPLATE = """\
# KeccakSim_v2 — {mode_desc}, smoke-v6 (SNR-equalized) noise sweep: sigma={sigma_f}.
# SNR_var={snr}/sigma^2~{snr_val:.4g}. Scale chosen to match HW SNR (Var_signal=2.0).
# Smoke: 10 ref, 10 det, 50 training, 20 validation sets, 50 SASCA traces.
# Comparable to smoke-v5 (HW/HD). Sweep step {step}/9.
SHA3_INPUTS=16
SHA3_INVOCATIONS=10
...
for sigma_s, sigma_f, step in SIGMAS:
    for mode, m in MODES.items():
        ...
        fname = os.path.join(SCRIPT_DIR, f".env_smoke_v6_{mode}_sigma{sigma_s}")
