...
Options:
  ...
  --skip-deploy       Skip sim/deploy only; run full 0001-0005 chain (Raw/ dirs must be pre-populated)
...
if [ "$SKIP_SIM" -eq 1 ] && [ "$SKIP_DETECTION" -eq 0 ] && [ "$SKIP_DEPLOY" -eq 0 ] && [ -z "${TRACES_DIR:-}" ]; then
...
elif [ "$SKIP_DETECTION" -eq 0 ] && [ "$SKIP_DEPLOY" -eq 0 ]; then
...
# --skip-detection/--skip-deploy: Raw/ dirs already populated; no simulation or deploy needed
