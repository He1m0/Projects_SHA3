...
lower=0
upper=$(PYTHONPATH="${PYTHONPATH_CFG}" "${PYTHON_BIN}" -c "import global_config as cfg; print(cfg.DETECTION_SET_COUNT)")
sets_per_part=$(PYTHONPATH="${PYTHONPATH_CFG}" "${PYTHON_BIN}" -c "import global_config as cfg; print(cfg.DETECTION_SETS_PER_PART)")
corr_bound=$(PYTHONPATH="${PYTHONPATH_CFG}" "${PYTHON_BIN}" -c "import global_config as cfg; print(cfg.DETECTION_CORR_BOUND)")
